...
if capture_mode == "Upload PCAP" and uploaded_file:
    with st.spinner("🔄 Decoding target network capture layers..."):
        ...
        # === ADD ALERT FOR UPLOADED PCAP ===
        if not df.empty:
            if df['anomaly'].sum() > 0:
                send_device_notification(
                    f"🔴 Anomaly Detected: {df['anomaly'].sum()} threats in PCAP", 
                    f"Uploaded PCAP file contains {df['anomaly'].sum()} anomalous packets out of {len(packets)} total.", 
                    "critical"
                )
            else:
                send_device_notification(
                    "✅ PCAP Loaded - No Threats", 
                    f"Uploaded file contains {len(packets)} packets with no anomalies detected.", 
                    "normal"
                )

elif capture_mode == "Live Sniffing":
    if st.button("🚀 Start Live Sniffing & Threat Hunting"):
        with st.spinner(f"Sniffing on {interface} for {duration} seconds..."):
            try:
                ...
                # === ADD ALERT FOR LIVE SNIFFING ===
                if threat_count > 0:
                    send_device_notification(
                        f"🔴 Live Threat Detected: {threat_count} suspicious packets", 
                        f"Live capture found {threat_count} packets exceeding 1000 bytes threshold. Total captured: {len(packets)} packets.", 
                        "critical"
                    )
                elif not df.empty and df['anomaly'].sum() > 0:
                    send_device_notification(
                        f"⚠️ Anomaly Detected: {df['anomaly'].sum()} statistical outliers", 
                        f"Z-score analysis found {df['anomaly'].sum()} anomalous packets.", 
                        "warning"
                    )
                else:
                    send_device_notification(
                        "✅ Live Capture Complete - No Threats", 
                        f"No suspicious packets detected in {len(packets)} captured packets.", 
                        "normal"
                    )
...
# Assign them to variables to use in your 'with' blocks
...
with tab1:
    ...
    if st.button("🚀 Start Live Interception", type="primary", use_container_width=True):
        packet_cards_list = []
        # Use session state packets or current
        display_packets = st.session_state.all_packets if st.session_state.all_packets else packets
        for i, pkt in enumerate(display_packets[:packet_count]):
            if IP in pkt:
                size = len(pkt)
                proto = "TCP" if TCP in pkt else "UDP" if UDP in pkt else "Other"
                src_ip = pkt[IP].src
                dst_ip = pkt[IP].dst
                is_anomaly = size > 1000
...
with tab2:
    ...
    # Use session state df or current
    display_df = st.session_state.captured_df if not st.session_state.captured_df.empty else df
    if not display_df.empty:
        display_packets = st.session_state.all_packets if st.session_state.all_packets else packets
        col1, col2, col3, col4 = st.columns(4)
        with col1: st.metric("Total Packets", len(display_packets))
        with col2: st.metric("Unique Host IPs", display_df['src_ip'].nunique())
        with col3: st.metric("Protocols", display_df['protocol'].nunique())
        with col4: st.metric("Total Network Volume", f"{display_df['size'].sum() / 1000:.2f} KB")
...
with tab3:
    ...
    if not display_df.empty:
        ...
        with col2:
            ...
            df_time = display_df.set_index('timestamp').resample('10s').size().reset_index(name='Packets')
    ...
    if st.button("🚨 Run Full SOC Triage Analysis", type="primary", use_container_width=True):
        ...
        with st.spinner("Structuring incident evaluation context blocks..."):
            ...
            try:
                ...
                # --- STEP C: Fallback (This ensures report_data is NEVER missing) ---
            ...
            # --- STEP D: Render UI (ONLY happens after button click) ---
